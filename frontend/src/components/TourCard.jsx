...
export default function TourCard({ id, title, price, duration, groupSize, image, rating, description }) {
  return (
    <Link to={`/tour/${id}`} className="block h-full">
      <motion.div 
        whileHover={{ y: -10 }}
        className="bg-amber-50/50 rounded-[2.5rem] overflow-hidden shadow-sm hover:shadow-xl transition-all border border-amber-200/50 group h-full flex flex-col cursor-pointer"
      >
        ...
        <div className="p-8 flex-grow flex flex-col">
          ...
          <div 
            className="w-full py-4 rounded-2xl border-2 border-brand-orange/20 text-brand-orange font-bold flex items-center justify-center gap-2 group-hover:bg-brand-orange group-hover:text-white transition-all"
          >
